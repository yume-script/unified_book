...
def extract_url_from_text(text):
    """검색어 문자열 안에 URL이 섞여 있으면 추출"""
    if not text:
        return None
    m = re.search(r'https?://[^\s\)\]\'"]+', text)
    return m.group(0) if m else None

def extract_isbn_from_link(url, timeout=8):
    """서점/쇼핑몰 상세페이지 URL에서 ISBN을 추출.
    1단계: OG 메타태그(books:isbn, og:isbn) 우선 파싱 (리디북스 등)
    2단계: 실패 시 페이지 본문 텍스트에서 ISBN 패턴 정규식 백업 (네이버쇼핑 등)"""
    if not url:
        return None
    try:
    ...
    # 1단계: OG 메타태그 (property/content 순서가 뒤바뀐 경우까지 대응)
    m = re.search(r'<meta[^>]+property=["\'](?:books:isbn|og:isbn)["\'][^>]+content=["\']([\dXx\-]+)["\']', html_data)
    if not m:
        m = re.search(r'<meta[^>]+content=["\']([\dXx\-]+)["\'][^>]+property=["\'](?:books:isbn|og:isbn)["\']', html_data)
    if m:
        clean = re.sub(r'[^0-9X]', '', m.group(1).upper())
        if validate_isbn13(clean) or validate_isbn10(clean):
            return clean

    # 2단계 백업: 본문 텍스트 내 ISBN 패턴 정규식 스캔 (네이버쇼핑 등 메타태그 없는 페이지 대응)
    text_only = re.sub('<[^<]+?>', ' ', html.unescape(html_data))
    isbn_pat = re.compile(r'\b(?:97[89][-\s]?)?\d{1,5}[-\s]?\d{1,7}[-\s]?\d{1,6}[-\s]?[\dX]\b')
    isbn10_candidates = []
    for match in isbn_pat.findall(text_only):
...
def extract_isbn_from_epub(epub_path):
    """EPUB 내부 컨테이너 구조 및 본문 파일 분석 후 ISBN 추출 (정규식 기반 로컬 판독 전용)"""
    try:
...
def extract_isbn_from_pdf(pdf_path):
    """PDF 메타데이터 및 전후면 판권 페이지 고속 스캔 (정규식 기반 로컬 판독 전용)"""
