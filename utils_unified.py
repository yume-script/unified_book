...
class UnifiedBookMetadataProvider(BaseMetadataProvider):
    ...
    config_schema = [
        {"key": "ALADIN_KEY", "label": "알라딘 TTBKey", "type": "text", "required": False, "description": "알라딘 Open API 서비스에서 발급받은 개인 TTBKey를 입력하세요."},
        {"key": "NAVER_ID", "label": "네이버 Client ID", "type": "text", "required": False, "description": "네이버 개발자 센터에서 애플리케이션 등록 후 발급받은 Client ID를 입력하세요."},
        {"key": "NAVER_SECRET", "label": "네이버 Client Secret", "type": "text", "required": False, "description": "네이버 개발자 센터에서 발급받은 Client Secret 보안키를 입력하세요."},
        {"key": "GOOGLE_API_KEY", "label": "Google API Key", "type": "text", "required": False, "description": "Google Cloud Console에서 활성화한 Books API Key를 입력하세요 (선택 사항)."},
        {"key": "STRICT_MATCH", "label": "검색 결과 엄격한 필터링", "type": "checkbox", "required": False, "description": "체크 시, 검색어와 제목이 일치하는 도서 결과만 엄격하게 대조하여 필터링합니다."},
        {"key": "ISBN_FILE_SCAN", "label": "도서 파일(EPUB/PDF) 내부에서 ISBN 검출 시도", "type": "checkbox", "required": False, "description": "체크 시, 수동 검색 시 실제 도서 파일을 열어 판권지 속 ISBN을 추적합니다 (저사양 서버는 해제 권장)."}
    ]
                ...
                if validate_isbn13(clean_db_isbn) or validate_isbn10(clean_db_isbn):
                    ...
                    if isbn_file_scan:
                        ...
                        if extracted_isbn:
                            ...
                            detection_source = method  # 감지출처: LOCAL
